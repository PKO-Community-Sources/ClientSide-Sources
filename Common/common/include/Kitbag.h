...
inline char* KitbagData2String(CKitbag *pKitbag, char *szStrBuf, int nLen)
{
	...
	for (int i = 0; i < enumKBITEM_TYPE_NUM; i++)
	{
		...
		for (int j = 0; j < pKitbag->GetCapacity(); j++)
		{
			...
			if (!pGridCont)
				continue;
			sprintf(szData, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
			...
			lnCheckSum += (pGridCont->sID + pGridCont->sNum + pGridCont->sEndure[0] + pGridCont->sEndure[1] + pGridCont->sEnergy[0] + pGridCont->sEnergy[1] + pGridCont->chForgeLv	+	pGridCont->dwDBID	+ pGridCont->sNeedLv + pGridCont->bIsLock);
...
inline bool String2KitbagData(CKitbag *pKitbag, std::string &strData)
{
	...
	for (int i = 0; i < sPageNum; i++)
	{
		...
		for (int j = 0; j < nItemNumPerPage; j++)
		{
			...
			lnCheckSum += SGridCont.sID + SGridCont.sNum + SGridCont.sEndure[0] + SGridCont.sEndure[1] + SGridCont.sEnergy[0] + SGridCont.sEnergy[1] + SGridCont.chForgeLv	+	SGridCont.dwDBID +SGridCont.sNeedLv	+ SGridCont.bIsLock;
					...
					)
{
	...
	//	打印道具信息到字符串。
	_snprintf(	szData,	256,	"%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
		iOrder,
		pGridCont->sID,
		pGridCont->sNum,
		pGridCont->sEndure[0],
