	...
	*/
	...
	for (int i = 0; i < enumEQUIP_NUM; i++)
	{
		...
		sprintf(szData, ";%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
			pGridCont->bIsLock, pGridCont->sNeedLv, pGridCont->dwDBID, pGridCont->sID, pGridCont->sNum,
			pGridCont->sEndure[0], pGridCont->sEndure[1], pGridCont->sEnergy[0], pGridCont->sEnergy[1], pGridCont->chForgeLv);
		//	2008-8-7	yangyinyu	change	end!
		nDataLen = (int)strlen(szData);
		if (nBufLen + nDataLen >= nLen) return NULL;
		...
		*/
		lnCheckSum += (pGridCont->bIsLock + pGridCont->sNeedLv + pGridCont->dwDBID + pGridCont->sID + pGridCont->sNum + pGridCont->sEndure[0] + pGridCont->sEndure[1] + pGridCont->sEnergy[0] + pGridCont->sEnergy[1] + pGridCont->chForgeLv);
		//	2008-8-7	yangyinyu	change	end!
