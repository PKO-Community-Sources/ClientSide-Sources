...
//#endif
	...
	if(FAILED(D3DXCreateTextureFromFileEx(m_pDev,
		"texture\\minimap\\mapmask.bmp", //文件名
		0, 
		0, 
		1, //需要多少级mipmap，这里设为1
		0, //此纹理的用途
		...
		D3DX_FILTER_TRIANGLE|D3DX_FILTER_MIRROR, //纹理过滤方法
	...
	if(FAILED(D3DXCreateTextureFromFileEx(m_pDev,
		"texture\\minimap\\mapsea.bmp", //文件名
		0, 
		0, 
		1, //需要多少级mipmap，这里设为1
		0, //此纹理的用途
			...
			}else
			{
				if((_pTex[n][m].x != _sx + n) || (_pTex[n][m].y != _sy + m))
				{
					...
					D3DXCreateTextureFromFileEx(m_pDev,
						...
						D3DFMT_UNKNOWN, //自动检测文件格式
						D3DPOOL_MANAGED, //由DXGraphics管理
						D3DX_FILTER_LINEAR, //纹理过滤方法 TRIANGLE|D3DX_FILTER_DITHER
						D3DX_FILTER_NONE, //mipmap纹理过滤方法
						0x00000000, //透明色颜色
						NULL, //读出的图像格式存储在何变量中
						NULL, //读出的调色板存储在何变量中
						&_pTex[n][m].pTex);//要创建的纹理
#endif
					if(_pTex[n][m].pTex)
					{
						_pTex[n][m].x	= _sx + n;
...
void	CLargerMap::RenderScene()
{
	...
	//Modify by sunny.sun 20080904 绵羊要求去掉探索度
